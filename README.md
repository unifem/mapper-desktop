...
[![Build Status](https://travis-ci.org/unifem/mapper-desktop.svg?branch=master)](https://travis-ci.org/unifem/mapper-desktop) [![](https://images.microbadger.com/badges/image/unifem/mapper-desktop.svg)](https://microbadger.com/images/unifem/mapper-desktop)
...
[`mapper_jupyter.py`](https://raw.githubusercontent.com/unifem/mapper-desktop/master/mapper_jupyter.py)
...
```
curl https://raw.githubusercontent.com/unifem/mapper-desktop/master/mapper_jupyer.py -outfile mapper_jupyer.py
```
On Linux or Mac, start a terminal, use the `cd` command to change to the working directory, and then run the following command:
```
curl -s -O https://raw.githubusercontent.com/unifem/mapper-desktop/master/mapper_jupyer.py
```

After downloading the script, you can start the Docker image using the command
```
python mapper_jupyer.py -p
```
...
```
python mapper_jupyter.py -h
```

### Running the Docker Image with Desktop Environment
Besides using the Docker Image as a Jupyter Notebook server, you can also start it in an X-Windows desktop environment. Simply replace `mapper_jupyter.py` with `mapper_desktop.py` in the preceding commands. That is, on Windows use the following commands in the PowerShell:
```
curl https://raw.githubusercontent.com/unifem/mapper-desktop/master/mapper_desktop.py -outfile mapper_desktop.py
python mapper_desktop.py -p
```
and on Linux and Mac use the following commands in a terminal:
```
curl -s -O https://raw.githubusercontent.com/unifem/mapper-desktop/master/mapper_desktop.py
python mapper_desktop.py -p
```
...
```
python mapper_jupyter.py
```
or
```
python mapper_desktop.py
```
